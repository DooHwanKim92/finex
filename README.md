...
## 미비사항 or 막힌 부분
- 로그인, 로그아웃 검증 → get 매핑된 메서드만 구현해놓으면 스프링 시큐리티의 .formLogin() , .logout()에서 알아서 해준다.
- 수정, 삭제에 대한 권한 검증 → Principal에서 현재 로그인된 사용자의 정보를 가져와서 비교 활용해야 한다.
- 검색 기능 구현 → 원리는 대충 파악, template 작성 아직 미숙